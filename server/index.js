...
async function generateWithHuggingFace(prompt) {
  const model = process.env.HF_MODEL || 'TinyLlama/TinyLlama-1.1B-Chat-v1.0';
  ...
  try {
    const response = await fetch(`${HF_API}/${model}`, {
      ...
      signal: controller.signal,
      body: JSON.stringify({
        inputs: `Write a detailed note about: ${prompt}`,
...
app.post('/api/ai/generate', async (req, res) => {
  ...
  try {
    ...
    console.error('[AI] Hugging Face failed:', err.message);
  ...
  try {
    ...
    lastError += ' | Ollama: ' + err.message;
  }

  console.error('[AI] Both providers failed:', lastError);
  res.status(503).json({
    error: 'AI generation unavailable',
    detail: `Last error: ${lastError}. Set HUGGINGFACE_API_KEY in Render env vars (free at hf.co/settings/tokens) for better reliability, or use a custom HF_MODEL.`,
  });
});
